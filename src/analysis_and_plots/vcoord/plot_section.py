...
def mpl_sec_bathy(rbat, rbat_fill, zenv, x2, tdep2, wdep2, tmsk2, mbat_fill, \
    ...
    if tdep2 != [] and wdep2 != [] and tmsk2 != [] and vlevel != "no":
       ...
       if mbat_fill == "true":
             ...
             codes[0]  = path.Path.MOVETO
             polyg     = path.Path(vertexes, codes)
             mpatch    = patches.PathPatch(polyg, facecolor='gray', edgecolor='gray', alpha=1.,zorder=5)
...
def mpl_sec_draw_levels(x2, wlev2, tmsk2):

    nk = wlev2.shape[0]

    #wlev_tmp = np.copy(wlev2)
    #wlev_tmp[tmsk2==0] = np.nan
   
    for k in np.arange(nk):

        W_l  = plt.plot(x2[k,:], wlev2[k,:], linestyle='-', color='k',zorder=10)
        #W_l  = plt.plot(x2[k,:], wlev_tmp[k,:], linestyle='-', color='k',zorder=10)
        plt.setp(W_l, 'linewidth', 0.35)

    #wlev_tmp = np.copy(wlev2)
    #wlev_tmp[tmsk2==1] = np.nan
...
def mpl_lev(exp_name, fig_type, sec_i, sec_j, tlon3, tlat3, tdep3, wdep3, tmsk3, m, \
            ...
            if i3D_flag:
               ...
               W_l = mpl_sec_draw_levels(x2, wlev2, tmsk2)
