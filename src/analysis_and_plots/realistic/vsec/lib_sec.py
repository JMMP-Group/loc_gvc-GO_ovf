...
def mpl_sec_bathy(rbat, rbat_fill, zenv, x2, tdep2, wdep2, tmsk2, mbat_fill, \
    ...
    if rbat_fill == "true":
       ...
       rpatch    = patches.PathPatch(polyg, facecolor='black', edgecolor='black', alpha=1.,zorder=20)
    ...
    if tdep2 != [] and wdep2 != [] and tmsk2 != [] and vlevel != "no":
       ...
       if mbat_fill == "true":
             ...
             codes[0]  = path.Path.MOVETO
             polyg     = path.Path(vertexes, codes)
             mpatch    = patches.PathPatch(polyg, facecolor='black', edgecolor='black', alpha=1.,zorder=10)
...
def mpl_sec_draw_scalar(VarName, PlotType, var2, x2, dep2, vlim_min, vlim_max, \
                        colmap, cn_line, cn_level, cn_label, cn_color, cn_fill, \
                        vcoord, var2_aux=[], var2_aux_lev=[], sec='dummy'):
    ...
    if PlotType == "contourf":    
       ...
       if var2_aux != []:
          ...
          #var2_aux_lev = [27.81, 27.82, 27.83, 27.84, 27.85, 27.86, 27.87, 27.88]

          pc_aux = plt.contour(x2, dep2, var2_aux_masked, var2_aux_lev, colors = 'k',       
                               linewidths=2., extend = 'both')

          #var2_aux_lev = [27.815, 27.825, 27.835, 27.845, 27.855, 27.865, 27.875]

          #pc_aux = plt.contour(x2, dep2, var2_aux_masked, var2_aux_lev, colors = 'w',
          #                     linewidths=2., extend = 'both')           
          ...
          if sec == 'osnap':
             ...
             ovf_rho = 27.84 
             DS_lim = 800.
             if vcoord == "SZT": DS_lim = 830.
             var2_msk_DS = var2_aux_masked              

             pc_ovf = plt.contour(x2, dep2, var2_msk_DS, [ovf_rho], colors = 'black', 
                                  linewidths=lw1, extend = 'both')
             pc_ovf1 = plt.contour(x2, dep2, var2_msk_DS, [ovf_rho], colors = ColLine,
                                  linewidths=lw2, extend = 'both')
             if vcoord != "no":
                plt.clabel(pc_ovf1, inline=0, fontsize=30, fmt='%1.2f', colors=ColText, zorder=11)
                           
             # Icelandic Basin
             var2_msk_IB = var2_aux_masked
             ovf_rho = 27.84 
             pc_ovf = plt.contour(x2, dep2, var2_msk_IB, [ovf_rho], colors = 'black',    
                                  linewidths=lw1, extend = 'both')
             pc_ovf1 = plt.contour(x2, dep2, var2_msk_IB, [ovf_rho], colors = ColLine,
                                  linewidths=lw2, extend = 'both')
             if vcoord != "no":
                plt.clabel(pc_ovf1, inline=0, fontsize=30, fmt='%1.2f', colors=ColText, zorder=11)
             ...
             ovf_rho = 27.8 
             pc_ovf = plt.contour(x2, dep2, var2_aux_masked, [ovf_rho], colors = 'black',
                                  linewidths=lw1, extend = 'both')
             pc_ovf1 = plt.contour(x2, dep2, var2_aux_masked, [ovf_rho], colors = ColLine,
                                  linewidths=lw2, extend = 'both')
             plt.clabel(pc_ovf1, inline=0, fontsize=30, fmt='%1.2f', colors=ColText, zorder=11)
             ...
             ovf_rho = 27.85 # Holliday et al. 2015 (10.1002/2015JC010762)
             pc_ovf = plt.contour(x2, dep2, var2_aux_masked, [ovf_rho], colors = 'black',
                                  linewidths=lw1, extend = 'both')
             pc_ovf1 = plt.contour(x2, dep2, var2_aux_masked, [ovf_rho], colors = ColLine,
                                  linewidths=lw2, extend = 'both')
...
def mpl_sec_figure(fig, pc, var_strng, title, xlabel, ylabel, cn_level=None):
    ...
    #   cb.set_label(var_strng, labelpad=20, size=30)
...
def mpl_lev(exp_name, fig_type, sec_i, sec_j, tlon3, tlat3, tdep3, wdep3, tmsk3, m, \
            ...
            if m != []:
               ...
               a = plt.axes([pos1.x0+0.5, pos1.y0+0.01, .2, .2])
               m.drawcoastlines()
               m.fillcontinents()
...
def mpl_sec(exp_name, fig_type, var_strng, unit_strng, date, time, timestep, PlotType, sec_i, sec_j,
            ...
            var_aux=None, var_aux_lev=None, msk_mes=None, first_zlv=None, figwidth=34.,figheight=17.5, sec='dummy'):
       ...
       if PlotType == "pcolor":
             ...
             if VLEV == "Z_ps":
                rpatch, mpatch, RB, MB, ZE = mpl_sec_bathy(rbat2_sec, rbat2_fill, zenv2_sec, \
                                                           ...
                                                           VLEV, PlotType)
             elif VLEV == "S_re" or VLEV == "MES" or VLEV == "SZT":
                #rbat2_max = np.nanmax(rbat2)
       ...
       if mbat_fill == "true": #and vlevel != "Z_ps":
          ...
          for i in np.arange(Tvar2_sol.shape[1]):
              for k in np.arange(1,Tvar2_sol.shape[0]):
                  ...
                  #   break
                  if np.isnan(Tvar2_sol[k::,i]).all():
                     Tvar2_sol[k::,i] = Tvar2_sol[k-1,i]
                     if Tvar2_auxsol != []: 
                        Tvar2_auxsol[k::,i] = Tvar2_auxsol[k-1,i]
                     break
          cmap, pc = mpl_sec_draw_scalar(exp_name, PlotType, Tvar2_sol, Tx2, Tdep2, vlim_min, vlim_max, \
                                         colmap, cn_line, cn_level, cn_label, cn_color, cn_fill, vlevel, \
                                         Tvar2_auxsol, var_aux_lev, sec)
               
    elif PlotType == "pcolor":
          cmap, pc = mpl_sec_draw_scalar(exp_name, PlotType, Tvar2, Tx2, Tdep2, vlim_min, vlim_max, \
                                         colmap, cn_line, cn_level, cn_label, cn_color, cn_fill, vlevel)
    else:
       cmap = []
       pc   = []
        ...
        if plot_geom:
           ...
           if isinstance(timestep, (str, bytes)):
              ...
              mpl_sec(exp_name, fig_type, var_strng, unit_strng, date, time, timestep, PlotType, \
                      ...
                      cn_RegVertLev, cn_RVlevel, var_aux, var_aux_lev)
           else:
              for t in timestep:
                  print("time step: ", t)
                  if var4 != []:
                  ...
                  mpl_sec(exp_name, fig_type, var_strng, unit_strng, date, time, str(t+1), PlotType, \
                          ...
                          var3_aux, var_aux_lev, msk_mes, first_zlv,figwidth,figheight, sec)
