...
ds_obs = ds_obs.where(np.logical_and(ds_obs.longitude>-31.2,ds_obs.longitude<=-13.2))  # only IB
...
ds_vflux.to_netcdf('vflux_rho-IB_obs.nc')
...
for e in range(len(exp)):
    ...
    ds = ds.where(np.logical_and(ds.nav_lon>-31.2,ds.nav_lon<=-13.2)) # only IB
    ...
    ds_vflux.to_netcdf('vflux_rho-IB_' + lab[e] + '.nc')
